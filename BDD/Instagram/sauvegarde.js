  ...
  if($_GET["action"] == "NomWafer_infoxy")
  {
    if(isset($_GET["Nom_Wafer"]))
    {
      $reponse = $bdd->prepare(
        ...
        WHERE composant.Nom_Wafer=:nomWafer AND composant.ID_Type_Reticule_Ref=type_reticule.ID_type_reticule AND composant.Pos_X_Ret=type_wafer.Coord_X_Ret AND composant.Pos_Y_Ret=type_wafer.Coord_Y_Ret;'
        );
...
/*SELECT Chemin, Extension FROM image INNER JOIN composant ON composant.ID_Composant = image.ID_Composant WHERE (composant.Pos_X_Ret*10 + composant.Pos_Y_Ret) =14*/
  ...
  if($_POST['action'] == "addClient")
  {
    if($_POST["name_client"] != "")
    {
      ...
      $req2 = $bdd->prepare('INSERT INTO client_expedition(Nom_Entreprise) VALUES (:Nom_Entreprise)');
      $req2->execute(array(
        'Nom_Entreprise' => $_POST["name_client"]
      ));
